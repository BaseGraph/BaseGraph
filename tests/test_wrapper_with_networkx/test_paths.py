...
def test_shortest_paths(small_directed_fixture, small_undirected_fixture):
    for bg_graph, nx_graph in small_directed_fixture+small_undirected_fixture:
        ...
        for i, vertex in enumerate(vertices):
            bg_path1 = [[vertices[k] for k in path] for path in metrics.find_geodesics_from_vertex_idx(bg_graph, i)]
            for j in bg_graph:
                if not (i<=j or nx_graph.is_directed()):  # Checking only i<=j for undirected graphs
                   break

                bg_path2 = [vertices[k] for k in metrics.find_geodesics_idx(bg_graph, i, j)]
...
def test_all_shortest_paths(small_directed_fixture, small_undirected_fixture):
    for bg_graph, nx_graph in small_directed_fixture+small_undirected_fixture:
        ...
        for i, vertex in enumerate(vertices):
            bg_path1 = [[[vertices[k] for k in path] for path in paths] for paths in metrics.find_all_geodesics_from_vertex_idx(bg_graph, i)]
            for j in bg_graph:
                if not (i<=j or nx_graph.is_directed()):  # Checking only i<=j for undirected graphs
                   break

                bg_path2 = [[vertices[k] for k in path] for path in metrics.find_all_geodesics_idx(bg_graph, i, j)]
