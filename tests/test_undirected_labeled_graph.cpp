...
TYPED_TEST(
    EdgeLabeledUndirectedGraph_, addEdge_validEdge_successorInAdjacency
) {
...
TYPED_TEST(
    EdgeLabeledUndirectedGraph_, addEdge_multiedge_successorInAdjacencyOnce
) {
...
TYPED_TEST(
    EdgeLabeledUndirectedGraph_, addEdge_vertexOutOfRange_throwInvalidArgument
) {
    BaseGraph::LabeledUndirectedGraph<TypeParam> graph(0);
...
TYPED_TEST(EdgeLabeledUndirectedGraph_, hasEdge_existentEdge_ReturnTrue) {
    ...
    for (auto edge_label : std::list<std::tuple<
             BaseGraph::VertexIndex, BaseGraph::VertexIndex, TypeParam>>{
...
TYPED_TEST(
    EdgeLabeledUndirectedGraph_, hasEdge_vertexOutOfRange_throwOutOfRange
) {
    BaseGraph::LabeledUndirectedGraph<TypeParam> graph(0);
    ...
    for (auto edge : std::list<BaseGraph::Edge>{{1, 2}, {2, 1}}) {
        ...
        EXPECT_THROW(
            graph.hasEdge(edge.first, edge.second, this->labels[0]),
            std::out_of_range
        );
    }
}
...
static void testAllEdgesExistForContainer(Labels &labels) {
    ...
    for (auto edge : edges) {
        ...
        EXPECT_TRUE(graph.hasEdge(
            std::get<1>(edge), std::get<0>(edge), std::get<2>(edge)
        ));
    }
...
TYPED_TEST(
    EdgeLabeledUndirectedGraph_, edgeListConstructor_anyContainer_allEdgesExist
) {
...
TYPED_TEST(
    EdgeLabeledUndirectedGraph_, getEdgeLabel_existentEdge_correctLabel
) {
    this->graph.addEdge(0, 2, this->labels[0]);
    this->graph.addEdge(0, 1, this->labels[1]);
...
TYPED_TEST(
    EdgeLabeledUndirectedGraph_, setEdgeLabel_existentEdge_labelChanged
) {
    this->graph.addEdge(0, 2, this->labels[0]);
    this->graph.addEdge(0, 1, this->labels[1]);
...
TYPED_TEST(
    ...
    EXPECT_THROW(
        this->graph.setEdgeLabel(0, 2, this->labels[0]), std::invalid_argument
    );
    this->graph.addEdge(0, 1, this->labels[0]);
    EXPECT_THROW(
        this->graph.setEdgeLabel(0, 2, this->labels[0]), std::invalid_argument
...
TYPED_TEST(
    EdgeLabeledUndirectedGraph_, removeEdge_existentEdge_edgeDoesntExist
) {
    this->graph.addEdge(0, 1, this->labels[0]);
...
TYPED_TEST(
    EdgeLabeledUndirectedGraph_, removeEdge_existentSelfLoop_edgeDoesntExist
) {
    this->graph.addEdge(0, 1, this->labels[0]);
...
TYPED_TEST(
    EdgeLabeledUndirectedGraph_, removeEdge_inexistentEdge_edgeDoesntExist
) {
    this->graph.addEdge(0, 1, this->labels[0]);
    this->graph.removeEdge(0, 2);
...
TYPED_TEST(
    EdgeLabeledUndirectedGraph_, removeDuplicateEdges_noMultiedge_doNothing
) {
    this->graph.addEdge(0, 1, this->labels[0]);
    this->graph.addEdge(0, 2, this->labels[1]);
...
TYPED_TEST(
    EdgeLabeledUndirectedGraph_, removeDuplicateEdges_multiedge_keepOneEdge
) {
    this->graph.addEdge(0, 1, this->labels[0]);
    this->graph.addEdge(0, 2, this->labels[1]);
...
TYPED_TEST(
    EdgeLabeledUndirectedGraph_, removeSelfLoops_existentSelfLoop_removeSelfLoop
) {
    this->graph.addEdge(0, 1, this->labels[0]);
    this->graph.addEdge(0, 2, this->labels[1]);
...
TYPED_TEST(
    EdgeLabeledUndirectedGraph_, equalityOperator_twoEmptyGraphs_returnTrue
) {
...
TYPED_TEST(
    EdgeLabeledUndirectedGraph_, equalityOperator_differentSize_returnFalse
) {
...
TYPED_TEST(
    EdgeLabeledUndirectedGraph_, equalityOperator_sameEdgesAndSize_returnTrue
) {
    BaseGraph::LabeledUndirectedGraph<TypeParam> graph(3);
...
TYPED_TEST(
    EdgeLabeledUndirectedGraph_, equalityOperator_differentEdgeOrder_returnTrue
) {
    BaseGraph::LabeledUndirectedGraph<TypeParam> graph(3);
    BaseGraph::LabeledUndirectedGraph<TypeParam> graph2(3);
    graph.addEdge(0, 2, this->labels[0]);
...
TYPED_TEST(
    EdgeLabeledUndirectedGraph_, equalityOperator_differentLabels_returnFalse
) {
    BaseGraph::LabeledUndirectedGraph<TypeParam> graph(3);
    BaseGraph::LabeledUndirectedGraph<TypeParam> graph2(3);
    graph.addEdge(0, 2, this->labels[0]);
...
TYPED_TEST(
    EdgeLabeledUndirectedGraph_, equalityOperator_missingEdge_returnFalse
) {
    BaseGraph::LabeledUndirectedGraph<TypeParam> graph(3);
    BaseGraph::LabeledUndirectedGraph<TypeParam> graph2(3);
...
TYPED_TEST(
    EdgeLabeledUndirectedGraph_, equalityOperator_differentEdges_returnFalse
) {
    BaseGraph::LabeledUndirectedGraph<TypeParam> graph(3);
    BaseGraph::LabeledUndirectedGraph<TypeParam> graph2(3);
    graph.addEdge(0, 1, this->labels[0]);
