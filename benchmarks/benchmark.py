...
def benchmark(func, library, n):
    ...
    print(f"{library}:\t{np.average(times)}±{np.std(times)/np.sqrt(n)} ms")
...
if __name__ == "__main__":
    ...
    bg_graph = bg.core.load_undirected_text_edgelist(edgelist_name)[0]
    # bg_graph = bg.random.generate_small_world_random_graph(10000, 2, 0.001)
    ...
    print("Benchmark - Shortest path lengths")
    benchmark(
        basic_timed_func(
            lambda: bg.metrics.get_shortest_path_lengths_from_vertex(bg_graph, source_vertex)
        ),
        basegraph_name, benchmark_iterations
    )
    benchmark(
        basic_timed_func(
            lambda: gt.shortest_distance(gt_graph, gt_vertices[source_vertex])
        ),
        graphtool_name, benchmark_iterations
    )
    benchmark(
        basic_timed_func(
            lambda: nx.algorithms.shortest_paths.generic.shortest_path_length(nx_graph, source=source_vertex)
        ),
    ...
    benchmark(
        basic_timed_func(lambda: gt.global_clustering(gt_graph)),
        graphtool_name, benchmark_iterations
    )
    benchmark(
    ...
    benchmark(
        basic_timed_func(lambda: gt.local_clustering(gt_graph)),
        graphtool_name, benchmark_iterations
    )
    benchmark(
        basic_timed_func(lambda: nx.clustering(nx_graph)),
        networkx_name, benchmark_iterations
    )
