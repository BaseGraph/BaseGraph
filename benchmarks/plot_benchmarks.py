...
benchmark_output = {
        "python": subprocess.run(["python", "benchmark.py"], stdout=subprocess.PIPE, text=True),
        "cpp": subprocess.run([os.path.join(file_directory, "build", "benchmark")
...
for language, output in benchmark_output.items():
    ...
    for line in output.stdout.splitlines():
        ...
        if regex_match:
            benchmark_results[benchmark_name][regex_match[1]] = (regex_match[2], regex_match[3])
    ...
    for i, (name, results) in enumerate(benchmark_results.items()):
        ...
        ax.bar(
            bin_start, lib_averages, yerr=lib_errors,
            width=width,
